...
- **THEN** 直近のイベント（Claim作成、Proposal作成、Review完了等）が時系列順で最大10件表示される
...
- **THEN** 当該Claimの変更イベント一覧がタイムライン形式で表示される（タイムスタンプ、イベント種別、Actor名、変更概要）
...
- **THEN** Claim/Conceptがノードとして、それらの関連・接続がエッジとして力指向グラフ（force-directed graph）で表示される
- **THEN** ノードの種類（Claim / Concept）が色またはアイコンで区別される

#### Scenario: Context別色分け

- **WHEN** グラフビューにClaimノードが表示される
- **THEN** 属するContext別にノードが色分けされ、異分野間の接続が視覚的に識別可能になる

#### Scenario: ノードの操作

- **WHEN** ユーザーがグラフ上のノードをクリックする
- **THEN** ノードの概要情報（Claimのstatement先頭部分、またはConceptのlabel）がポップアップ表示される
- **THEN** ポップアップから詳細画面へのリンクが提供される

#### Scenario: フィルタリング

- **WHEN** ユーザーがグラフビューのフィルターでContextまたは分野を選択する
- **THEN** 選択されたContext/分野に関連するノードとエッジのみが表示される

#### Scenario: 接続種別のエッジ表示

- **WHEN** Cross-field Connectionがエッジとして表示される
- **THEN** connection_type（equivalent / analogous / generalizes / contradicts / complements）に応じてエッジの色またはスタイル（実線/破線等）が異なる
